...
for iters in range(10):
    ...
    #only body
    recording_body = np.zeros((3, 4))
    video_body = np.zeros((3, 4))

    body_squeleton = [11,12,23,24]
    for i, j in zip(body_squeleton, range(len(body_squeleton))):
        recording_body[:,j] = recording[:,i]
        video_body[:,j] = video[:,i]

    #obtaining the transformation matrices
    R, t = rigid_transform_3D(recording_body, video_body)


    json_object = json.dumps(R.tolist()) 
    with open('../FrameMatrices/R_body'+ str(index) +'.json', "w") as outfile:
        outfile.write(json_object)

    json_object = json.dumps(t.tolist()) 
    with open('../FrameMatrices/t_body '+ str(index) +'.json', "w") as outfile:
        outfile.write(json_object)

    #visualizing the difference between original features and transformed ones
    plot3DKps(recording_body, video_body)
    res = (R@recording_body) + t
    plot3DKps(video_body, res)
