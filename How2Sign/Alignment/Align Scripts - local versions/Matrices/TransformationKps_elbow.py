...
for iters in range(10):
    ...
    #obtaining the transformation matrices (only elbow)
    R, t = rigid_transform_3D(recording[:,13:15], video[:,13:15])

    json_object = json.dumps(R.tolist()) 
    with open('../FrameMatrices/R_codo'+ str(index) +'.json', "w") as outfile:
        outfile.write(json_object)

    json_object = json.dumps(t.tolist()) 
    with open('../FrameMatrices/t_codo'+ str(index) +'.json', "w") as outfile:
        outfile.write(json_object)

    #visualizing the difference between original features and transformed ones
    plot3DKps(recording[:,13:15], video[:,13:15])
    res = (R@recording[:,13:15]) + t
    plot3DKps(video[:,13:15], res)